...
extern State gsGoal;
extern bool enough;
extern State movingObject;
bool movingObjectTime = false;
bool firstmap = true;
bool secondmap = false;
...
void PathControl::ApplyControls(State& start, State& end, Path controls)
{
    ...
    for (int j = 0; j < controls.size; ++j)
    {
        ...
        while (t >= interval)
        {
            if (sqrt(pow(X-movingObject.x,2)+pow(Y-movingObject.y,2)) > 60)
            {
            ...
            X = XConvertToPixel(x);
            Y = YConvertToPixel(y);      
            if (sqrt(pow(X-171,2)+pow(Y-281,2)) <= 100.0 && !enough && firstmap)
            {
            ...
            if (sqrt(pow(X-274,2)+pow(Y-213,2)) <= 100.0 && !enough && secondmap)
            {
            ...
            if (sqrt(pow(X-500,2)+pow(Y-250,2)) <= 210.0 && !enough && !movingObjectTime)
            {
                movingObjectTime = true;
                movingObject.x = 440;
                movingObject.y = 303;
            }
            cv::Point b(X, Y);
            ...
            // orientation
            long double newx = x + 0.5 * cos(z + M_PI_2);
            long double newy = y + 0.5 * sin(z + M_PI_2);
            ...
            if(movingObjectTime)
            {
                cv::Point m((int)movingObject.x, (int)movingObject.y);
                circle(temp, m, 10, cv::Scalar(0, 0, 0), -1, cv::LINE_AA); 
                if (movingObject.y < 80)
                {
                    movingObject.x += 0.2;
                }
                else if (movingObject.x <= 327 && movingObject.y <= 190) 
                {
                    movingObject.y -= 0.2;
                }
                else
       ...
       // myVideo->write(image);
        ...
        circle(temp, b, 6, cv::Scalar(0xFF, 128, 0x00), -1, cv::LINE_AA); 
        // orientation
        long double newx = x + 1 * cos(z + M_PI_2);
        long double newy = y + 1 * sin(z + M_PI_2);
        int newX = XConvertToPixel(newx);
        int newY = YConvertToPixel(newy);
        cv::Point end(newX, newY);
        line(temp, b, end, cv::Scalar(0xFF, 128, 0x00),2, cv::LINE_AA);
