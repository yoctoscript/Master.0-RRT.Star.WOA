...
State PathPlanner::Steer(State* sNear, State& sRand)
{
    ...
    SteerOptimizer so(*sNear, sRand, this->sGoal);
    State sBest = so.Apply();
    long double dx = sBest.x - sNear->x;
    long double dy = sBest.y - sNear->y;
    long double mag = std::sqrt(pow(dx, 2.0l) + pow(dy, 2.0l));
    long double unitX = dx / mag;
    long double unitY = dy / mag;
    long double x = sNear->x + this->stepSize * unitX;
    long double y = sNear->y + this->stepSize * unitY;
    if (x < myMap.origin.x)
    {
        x = myMap.origin.x;
    } 
