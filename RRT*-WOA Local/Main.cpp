...
cv::VideoWriter* myVideo1; /// Object used to write videos.
...
int main(int argc, char** argv){
    ...
    cv::Mat image = cv::imread(mapName, cv::IMREAD_GRAYSCALE);
    ...
    log.Info("RRT* => Built in: {:.3f} seconds", (long double)rElapsedTime.count() / 1000000.0l);
    log.Info("Path length = {:.2f} meters", path.array[path.size-1].c);
    rrt_star.Render(path);

    log.Info("Smoothing path ...");
